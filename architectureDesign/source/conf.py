...
htmlhelp_basename = 'shattuckite-design'
latex_engine = 'xelatex'
latex_documents = [
    (master_doc, 'shattuckite-design.tex', 'shattuckite 设计文档','SHADOC-003,SDP,第五组'
    ,'howto'),
]
...
with subprocess.Popen(['git', 'log', '--grep', 'SHADOC-{0:s} DOC UPDATE'.format(SHADOC_INDEX)], stdout=subprocess.PIPE) as proc:
    ...
    for i in result:  # type:str
        ...
        res['Note'] = i[5].lstrip()
        with subprocess.Popen(['git', 'describe', '--always',res['Hash']], stdout=subprocess.PIPE) as proc2:
...
texinfo_documents = [
    (master_doc, 'shattuckite', 'shattuckite 设计文档',
